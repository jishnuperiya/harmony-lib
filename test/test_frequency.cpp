...
{  
  frequency f1(440.0);
  CHECK(doctest::Approx(f1.hz()) == 440.0);
  CHECK_EQ(f1.midi(),69);
}
...
TEST_CASE("compound assignment operators for frequency class")
{
  frequency f1(440.0);
  f1+=20.0;
  CHECK(doctest::Approx(f1.hz()) == 460.0);
  
  f1-=10.0;
  CHECK(doctest::Approx(f1.hz()) == 450.0);
}
...
TEST_CASE("non-member arithmetic operations")
{
  frequency f1(440.0);  
  frequency f2 = f1*2.0;
  CHECK(doctest::Approx(f2.hz()) == 880.0);

  frequency f3 = f2/2.0;
  CHECK(doctest::Approx(f3.hz()) == 440.0);
}
